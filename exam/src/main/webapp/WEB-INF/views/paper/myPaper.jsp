            ...
            <table class="table table-hover table-bordered table-striped table-condensed">
                ...
                <tbody>
                    <c:forEach items="${page.row}" var="item" varStatus="v">
                        ...
                        <td>${item.remarks}</td>
                        <td>${item.examScore.score}</td>
                        <td>${item.examScore.rights}</td>
                        <td>${item.examScore.errors}</td>

                        <td class="col-md-3">
                            <a href="/exam/${item.paperid}/check.action?papername=${item.papername}" class="btn btn-success">查看解析</a>
                        </td>
                        </tr>
                    </c:forEach>
                    <tr>
